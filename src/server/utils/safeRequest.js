...
const validateRequiredRestrictedPresent = (restrictedAttributeAdditions) => {
  ...
  Object.keys(requiredRestrictedAttributes).forEach((key) => {
    ...
    && restrictedAttributeAdditions[key].every(
      (requiredAttibute) => restrictedAttributeAdditions[key].includes(requiredAttibute));
...
export const validateSafeRequestRestrictedAttributes = (requiredAttributes, moduleName) => {
  ...
  Object.keys(requiredAttributes)
    ...
    .forEach((key) => {
      ...
      const requestItemsIncluded = requiredItems.every(
        (requiredValue) => restrictedRequestAttributes[key].includes(requiredValue));
